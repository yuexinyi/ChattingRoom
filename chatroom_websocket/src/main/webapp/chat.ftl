...
<script type="text/javascript">
    ...
    if ('WebSocket' in window) {
        webSocket = new WebSocket('ws://localhost:8088/websocket?username=' + '${username}');
    } else {
        ...
        if (undefined != msg.names) {
            $.each(msg.names, function (key, value) {
                var htmlstr = '<li>'
                        ...
                        + '<label for="' + key + '"></label>'
                        + '</div>'
                        + '<div class="liLeft"><img src="assets/img/bx.png"/></div>'
                        + '<div class="liRight">'
    ...
    function send() {
        ...
        $("#dope").val("");
            ...
            //发送消息
            var htmlstr = '<li><div class="answerHead"><img src="assets/img/me.png"></div><div class="answers">'
